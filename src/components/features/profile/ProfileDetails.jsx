    ...
    <div className="space-y-4 w-full pb-5 border-b border-white/10">
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-4">
          <h1 className="font-semibold text-2xl">{user?.user?.display_name}</h1>
          <p className="bg-[#FF9A354D] text-xs px-3 py-1.5 rounded-full">
            Premium
          </p>
        </div>
        <p className="text-white/40 text-xs">@{user?.user?.username}</p>
      </div>
      <p className="text-xs font-light max-w-[420px]">{user?.user?.bio}</p>
      <div>
        <Link to={`https://x.com/${user?.user?.twitter_id}`} target="_blank">
